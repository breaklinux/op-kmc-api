...
def sshLinux(host, port, username, password, hostname, ip, logsize, registries, deploy):
    ...
    if deploy == 1 or deploy == 2 or deploy == 3:
        ...
        cmd = os_kernel_upgrade.upgradeKernel()
        for upgrade in cmd:
            sshCmd(ssh_client, upgrade)
        ...
        startK8sNodeDeployTime = runTime()
        if deploy == 3:
            print("node环境部署中......")
            cmd_list = []
            ...
            for c in cmd_list:
                sshCmd(ssh_client, c)
        endK8sNodeDeployTime = runTime()
        runTimeCalculate("k8s-master部署耗时: ", endK8sNodeDeployTime, startK8sNodeDeployTime)
